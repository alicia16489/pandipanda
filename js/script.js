			...
			admin_panel : {
				...
				list_users : function (data) {
					...
					for(var i=0; i<data['users'].length; i++){
						$("#list_membre table").append('<tr id=user_'+data['users'][i]['id']+'><td>'+data['users'][i]['login']+'</td><td>'+data['users'][i]['email']+'</td><td><select class="select_rank_user" id="user_'+data['users'][i]['id']+'_rank"></select></td></tr>');
						$("#user_"+data['users'][i]['id']).hide();
						$("#user_"+data['users'][i]['id']).fadeIn();
						
						for(var j=0; j<data['ranks'].length; j++){
							var pick = "";
							$('#user_'+data['users'][i]['id']+'_rank').append('<option value="'+data['ranks'][j]['id']+'">'+data['ranks'][j]['name']+'</option>');
						}
					}
				}
			},
